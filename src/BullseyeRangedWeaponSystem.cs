    ...
    public class BullseyeRangedWeaponSystem : ModSystem
    {
        ...
        public override void StartClientSide(ICoreClientAPI api)
        {
			...
			capi.Event.AfterActiveSlotChanged += (changeEvenArgs) => {
				if (changeEvenArgs.ToSlot < capi.World.Player.InventoryManager?.GetHotbarInventory().Count && capi.World.Player.InventoryManager?.GetHotbarItemstack(changeEvenArgs.ToSlot)?.Item is ItemRangedWeapon rangedWeapon)
				{
					rangedWeapon.GetBehavior<CollectibleBehaviorAnimatableSimpleWithAttach>()?.SetAttachedRenderInfo(null);
				}
			};
        }
		...
		// rangedChargeStartByEntityId is NOT synchronised between server and client!! Each will have different values
