...
namespace Bullseye
{
    public class ItemSlotAmmo : ItemSlot
    {
        public ItemSlotAmmo(InventoryBase inventory, bool isBuyingSlot = false) : base(inventory)
        {
        }
        ...
        public override string GetStackDescription(IClientWorldAccessor world, bool extendedDebugInfo)
        {
			...
			if (inventory is InventoryAmmoSelect inventoryAmmoSelect && inventoryAmmoSelect.PlayerEntity?.RightHandItemSlot?.Itemstack?.Item is ItemRangedWeapon itemRangedWeapon)
			{
