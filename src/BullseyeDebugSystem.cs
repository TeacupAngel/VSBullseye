...
namespace Bullseye
{
    public class BullseyeDebugSystem : ModSystem
    {
		...
		private void AutofireTick(float dt)
		{
			...
			switch (autofireStage)
			{
				case 0:
					...
					{
						BullseyeRangedWeaponSystem rangedWeaponSystem = capi.ModLoader.GetModSystem<BullseyeRangedWeaponSystem>();

						if (capi.World.Player.Entity?.RightHandItemSlot.Itemstack?.Collectible is ItemRangedWeapon rangedWeaponItem)
						{
							...
							rangedWeaponSystem.EntitySetAmmoType(capi.World.Player.Entity, rangedWeaponItem.AmmoCategory, ammoTypes[autofireAmmoType]);
							rangedWeaponSystem.SendRangedWeaponAmmoSelectPacket(rangedWeaponItem.AmmoCategory, ammoTypes[autofireAmmoType]);
						}
					}
